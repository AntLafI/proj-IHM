...
void RenderArea::undo(){
    ...
    if (!m_currentRect.isNull()) {
        m_redoRects.append(m_currentRect);
        m_currentRect = QRect();
        update();
        notifyState();
        emit modified();
    ...
    if (!m_rects.isEmpty()) {
        m_redoRects.append(m_rects.takeLast());
        update();
        notifyState();
        emit modified();
        return;
...
void RenderArea::redo(){
    ...
    if (!m_redoRects.isEmpty()) {
        m_rects.append(m_redoRects.takeLast());
        update();
        notifyState();
        emit modified();
        return;
...
void RenderArea::eraseAt(const QPoint &pt){
    ...
    for (int i = m_rects.size() - 1; i >= 0; --i) {
        if (m_rects[i].contains(basePt)) {
            EraseOp op;
            op.kind = EraseOp::Rect;
            op.index = i;
            op.rect = m_rects[i];
            m_rects.remove(i);
            m_eraseHistory.append(op);
